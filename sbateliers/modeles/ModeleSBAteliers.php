<?php
	...
	class ModeleSBAteliers {
		...
		private function __construct(){
			self::$connexion = new PDO( 'mysql:host=localhost;dbname=sb', 'sanayabio', 'azerty' ) ;
		}
...
FIN_REQ_ATELIERS_EN_COURS;
		...
		public static function saveCleSecrete($identifiantClient , $cleSecrete ){
			$bd = self::getConnexion() ;
			$cleS ="UPDATE client SET cle_secrete = :cleSecrete WHERE numero=:client";
			$st =$bd -> prepare ($ql) ;
			$st -> execute (array( ':client'=> $identifiantClient,':cleSecrete'=>$cleSecrete));
